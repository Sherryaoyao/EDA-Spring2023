...
```{r}
#type cars$ tab and you can select the columns in cars
cars$speed
```
...
How could you add columns to *df_car_speed*?（"**"italicize; "****"bold)
...
summary(df)
head(df) #show the first 6 rows of df; head(df,n=10)

#If you know the number of the column you want you can refer to that to access column, [1,] --> first row
df[,1]
#a<-df[10:20, 1:2]

#you could also use this notation to delete columns
df2 <- df[,-2] #deleting the second column
df2
df
```
